...
const DARK = {
  ...
  heroGrad:
    "linear-gradient(135deg, rgba(61,140,95,0.2) 0%, rgba(59,125,216,0.2) 100%)",
...
const makeCSS = (t) => `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  ::-webkit-scrollbar-thumb { background:${t.textMuted}; border-radius:4px; opacity:.4; }
...
function LoginScreen({ onLogin, theme, toggleTheme }) {
  ...
  const doLogin = () => {
    if (pw === sel.password) onLogin(sel);
    ...
    >
      ...
      <div className="fade-up" style={{ width: "100%", maxWidth: "380px" }}>
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <h1 style={{ color: t.text, fontSize: "28px", fontWeight: 800, letterSpacing: "-0.5px" }}>
            Finchly
          </h1>
          <p style={{ color: t.textSub, fontSize: "14px", marginTop: "5px" }}>
            Suas finanças, com leveza
          </p>
        </div>
        ...
        <Card style={{ padding: "28px" }}>
          ...
          {step === "register" && (
            <>
              ...
              <p style={{ fontSize: "12px", color: t.textSub, marginBottom: "9px", fontWeight: 500 }}>
                Escolha um avatar
              </p>
                ...
                {avatars.map((a) => (
                  <button
                    ...
                    style={{
                      fontSize: "22px",
                      padding: "7px 8px",
                      background: form.avatar === a ? t.accentSoft : "transparent",
                      border:
                        form.avatar === a
                          ? `2px solid ${t.accent}`
                          : `2px solid ${t.border}`,
              ...
              {err && (
                <p style={{ color: t.negative, fontSize: "13px", marginBottom: "9px" }}>
                  {err}
                </p>
              )}

              <div style={{ display: "flex", gap: "8px" }}>
                <Btn variant="ghost" onClick={() => setStep("choose")} style={{ flex: 1 }}>
                  Voltar
                </Btn>
                <Btn onClick={doRegister} style={{ flex: 2 }}>
                  Criar conta
                </Btn>
              </div>
            </>
          ...
          {step === "login" && sel && (
            <>
              <div style={{ textAlign: "center", marginBottom: "22px" }}>
                ...
                <h2 style={{ color: t.text, fontSize: "20px", fontWeight: 700 }}>
                  Olá, {sel.name}! 👋
                </h2>
                <p style={{ color: t.textSub, fontSize: "13px", marginTop: "4px" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <Btn onClick={doLogin} style={{ flex: 2 }}>
                  Entrar
                </Btn>
              </div>
            </>
        ...
        <p style={{ color: t.textMuted, fontSize: "12px", textAlign: "center", marginTop: "20px" }}>
          Finchly · Finanças com leveza 🌿
        </p>
      </div>
    </div>
  );
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            <p style={{ fontSize: "11px", color: t.textMuted, letterSpacing: "0.5px", fontWeight: 500 }}>
              Finchly
            </p>
        ...
        {tabs.map((tt) => (
          ...
          >
            <span
              style={{
                fontSize: "18px",
                filter: tab === tt.id ? "none" : "grayscale(1) opacity(0.5)",
              }}
            >
              {tt.icon}
            </span>
...
function Debts({ d, save }) {
  ...
  const add = () => {
    if (
      ...
      return setErr("Preencha os campos obrigatórios (*).");
    }
  ...
  const updateRem = (id) => {
    ...
    save({
      debts: d.debts.map((dbt) =>
        dbt.id === id ? { ...dbt, remainingInstallments: v } : dbt
      ),
    });
  ...
  const remove = (id) => {
    save({ debts: d.debts.filter((dbt) => dbt.id !== id) });
  };
  ...
  const totalPending = active.reduce(
    (s, dbt) => s + Number(dbt.installmentValue) * Number(dbt.remainingInstallments),
    0
  );
    ...
    <div className="fade-up">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
        <div>
          ...
          <p style={{ color: t.textSub, fontSize: "12px" }}>
            Valor restante:{" "}
            <span style={{ color: t.negative, fontFamily: "'JetBrains Mono'", fontWeight: 700 }}>
        ...
        <button
          ...
          style={{
            background: showForm ? t.bgInput : "linear-gradient(135deg,#E05A4A,#C0392B)",
      ...
      <Card
        style={{
          ...
          border: `1px solid ${t.accentBlue}30`,
        }}
      >
        <p style={{ fontSize: "12px", color: t.accentBlue, fontWeight: 500 }}>
          ℹ️ Dívidas são para controle pessoal — não afetam seu saldo ou gasto diário.
        </p>
      </Card>
      ...
      {showForm && (
        <Card style={{ padding: "18px", marginBottom: "15px" }}>
          ...
          {[
            ["text", "Descrição (ex: Cartão Nubank) *", "description"],
            ["text", "Credor / Banco", "creditor"],
            ["number", "Valor total da dívida (R$) *", "totalAmount"],
            ["number", "Valor de cada parcela (R$) *", "installmentValue"],
            ["number", "Total de parcelas *", "totalInstallments"],
            ["number", "Parcelas restantes *", "remainingInstallments"],
            ["number", "Dia do vencimento (ex: 10)", "dueDay"],
      ...
      {active.length === 0 && done.length === 0 ? (
        <div style={{ textAlign: "center", padding: "36px 20px", color: t.textMuted }}>
          ...
          <p style={{ fontSize: "13px" }}>Nenhuma dívida registrada!</p>
        </div>
      ) : (
        <>
          {active.map((dbt) => (
